...
// Lists local drafts — including any Claude created via the MCP connector, which
// carry a badge. This is where create_draft becomes visible to the user.
export function DraftsModal({ onClose, onEdit }: { onClose: () => void; onEdit: (d: DraftSummary) => void }): JSX.Element {
  ...
  return (
    <div className="absolute inset-0 z-[64] flex items-center justify-center" style={{ background: 'rgba(5,6,10,0.55)', backdropFilter: 'blur(3px)' }} onClick={onClose}>
      <div data-testid="drafts-modal" className="flex max-h-[80vh] w-[min(620px,92vw)] flex-col overflow-hidden rounded-lg border border-border bg-panel shadow-raised" onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-none items-center border-b border-border px-5 py-4">
          <div className="text-[16px] font-bold">Drafts</div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {drafts === null ? (
            ...
            <div className="px-2 py-10 text-center">
              <div className="text-[14px] font-bold text-text-2">No drafts</div>
            ...
            <div className="flex flex-col gap-1.5">
              {drafts.map((d) => (
                <div key={d.id} className="flex items-center gap-3 rounded-md border border-border bg-bg px-3.5 py-2.5">
                  ...
                  <button onClick={() => onEdit(d)} className="rounded-md px-2 py-1 text-[12px] font-semibold text-accent hover:underline">Edit</button>
