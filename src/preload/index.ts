...
const api: DeskMailApi = {
  ...
  mail: {
    ...
    canBackfill: (folderId: number) => ipcRenderer.invoke('mail:can-backfill', folderId),
    backfill: (folderId: number) => ipcRenderer.invoke('mail:backfill', folderId),
    syncDepthGet: () => ipcRenderer.invoke('mail:sync-depth-get'),
    syncDepthSet: (days: number) => ipcRenderer.invoke('mail:sync-depth-set', days),
